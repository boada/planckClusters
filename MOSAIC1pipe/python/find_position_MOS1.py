...
class finder:
    ...
    def read_cat(self):
        ...
        (ra,    dec,
         ...
         i_bpz, i_berr,
         class_star,
         a_image, b_image, theta,
         x_image,y_image) = tableio.get_data(self.catsfile,cols=cols)
    ...
    def get_absmags(self):
        ...
        sout.write("# Computing absolute magnitudes interpolating konly from BC03 model \n")
    ...
    def get_BCG_candidates(self, Mr_limit=-22.71, p_lim=1e-4):
        ...
        Mi_limit = cosmology.reobs('El_Benitez2003',m=Mr_limit, oldfilter="r_MOSAICII", newfilter="i_MOSAICII")
    ...
    def jpg_read(self,dx=1200,dy=1200):
        ...
        self.fitsfile = os.path.join(self.datapath,self.ctile,self.ctile + 'i.fits')
        self.jpgfile  = os.path.join(self.datapath,self.ctile,self.ctile+'.tiff')
        t0 = time.time()
    ...
    def jpg_display(self):
        ...
        pylab.savefig(self.figAname,transparent=True, dpi=100, bbox_inches='tight')
    ...
    def get_object(self,event):
        ...
        if event.key == 'c':
            ...
            z_cl,z_err = self.select_members_radius(self.iclose,radius=radius,zo=self.zo)
            z_cl,z_err = self.select_members_radius(self.iclose,radius=radius,zo=z_cl)
        ...
        if event.key == 'v':
            iclose = self.iclose
            text = " z_cl = %.3f\n zBCG = %.3f\n Ngal = %d\n R  = %d[kpc]" % (self.z_cl, self.z_ph[iclose],self.Ngal,self.radius)
            xo = 80
            yo = 80
            pylab.text(xo+2,self.ny - yo+2,text,color='black',fontsize=18)
        ...
        if event.key == 'w':
            ...
            self.write_redshift()
            self.write_members()
            pylab.savefig(self.figBname,transparent=True, dpi=100, bbox_inches='tight')
            sys.exit()
            return
        # Print info
        self.print_info()
    ...
    def select_members(self,i,Mi_lim=-20.25):
        ...
        r1Mpc = astrometry.kpc2arc(zo,R1Mpc,self.cosmo) / 3600. # in degrees.
        rcore = r1Mpc/2.0
        dist  = astrometry.circle_distance(ra0,dec0,self.ra,self.dec,units='deg')
        mask_R1Mpc = numarray.where(dist <= r1Mpc, 1, 0)
        mask_rcore = numarray.where(dist <= rcore, 1, 0)
        arcmin2Mpc = astrometry.arc2kpc(zo,60.0,self.cosmo)/1000.0 # scale between arcmin and Mpc
        ...
        mask_L1   = numarray.where(self.Mi <=  Mi_lim_zo, 1 , 0) # Faint  cut > 0.4L*
        ...
        while not converge:
            ...
            c1 = numarray.abs( gr[idc] - gr[idc].mean()) > Nsigma*numarray.std(gr[idc],ddof=1)
            c2 = numarray.abs( ri[idc] - ri[idc].mean()) > Nsigma*numarray.std(ri[idc],ddof=1)
            iclip = numarray.where( lor(c1,c2))  # where any of the conditions fails
            if len(iclip[0]) > 0:
        ...
        c1 = numarray.abs( self.gr - gr[idc].mean()) > Nsigma*numarray.std(gr[idc],ddof=1)
        c2 = numarray.abs( self.ri - ri[idc].mean()) > Nsigma*numarray.std(ri[idc],ddof=1)
        mask_cm = numarray.where( lor(c1,c2), 0 ,1) # where condition fails
        iR1Mpc = numarray.where ( mask_R1Mpc * mask_L1 * mask_L2 * mask_z * mask_cm == 1)
        Ngal = len(iR1Mpc[0])
        ...
        r200 = astrometry.kpc2arc(zo,R200*1000.0,self.cosmo) / 3600. # in degrees.
        mask_r200 = numarray.where(dist <= r200, 1, 0)
        i200 = numarray.where ( mask_r200 * mask_L1 * mask_L2 * mask_z * mask_cm == 1)
        N200 = len(i200[0])
        self.i200   = i200
        self.N200   = N200
        self.R200   = R200
    ...
    def select_members_radius(self,i,Mi_lim=-20.25,radius=500.0,zo=None):
        ...
        while not converge:
            ...
            c1 = numarray.abs( gr[idc] - gr[idc].mean()) > Nsigma*numarray.std(gr[idc],ddof=1)
            c2 = numarray.abs( ri[idc] - ri[idc].mean()) > Nsigma*numarray.std(ri[idc],ddof=1)
            iclip = numarray.where( lor(c1,c2))[0]  # where any of the conditions fails
            if len(iclip) > 0:
        ...
        c2 = numarray.abs( self.ri - ri[idc].mean()) > Nsigma*numarray.std(ri[idc],ddof=1)
        mask_cm = numarray.where( lor(c1,c2), 0 ,1) # where condition fails
        iRadius = numarray.where ( mask_R * mask_L1 * mask_L2 * mask_z * mask_cm == 1)
        Ngal = len(iRadius[0])
    ...
    def ellipse_members(self,k=0):
        ...
        for i in self.iRadius[0]:
            ...
            if i == self.iclose:
                ...
                ec = 'red'
            E = PEllipse((xo,yo),(a,b),resolution=80,angle=theta,fill=0, edgecolor=ec,linewidth=1.0)
            self.ellipse[i] = E
            ax.add_patch(E)
        ...
        C = PCircle((Xo,Yo),r_pixels,resolution=80,fill=0, edgecolor="white", linestyle='dashed', linewidth=0.5)
        ax.add_patch(C)
    ...
    def ellipse_BCGs(self):
        ...
        for i in self.idx_BCG[0]:
            ...
            yo = ny - yo
            ec = 'red'
            E = PEllipse((xo,yo),(a,b),resolution=80,angle=theta,fill=0, edgecolor=ec,linewidth=0.5)
            self.ellipse[i] = E
            ax.add_patch(E)
    ...
    def write_info(self):
        ...
        s = open(filename,"w")
        head = "# %-18s %12s %12s %7s %7s %5s %10s %10s %8s %8s %8s %8s %8s %8s %8s\n" % ('ID_BCG','RA','DEC','zBCG','z_cl','Ngal','L_i','L_iBCG','Mr','Mi','r','i','p_BCG','R[kpc]','area[%]') 
    ...
    def handle_ellipses(self,event,figure=1):
        ...
        if event.key == 'd' or event.key == 'D':
            ...
            self.ellipse[i] = PEllipse((xo,yo),(a,b),resolution=100,angle=theta,fill=0, edgecolor="yellow")
            current_ell     = PEllipse((xo,yo),(a,b),resolution=100,angle=theta,fill=0, edgecolor="white")
    ...
    def write_redshift(self):
        ...
        filename =  os.path.join(self.datapath,self.ctile,self.ctile + ".redshifts")
    ...
    def plot_probs(self):
        ...
        pylab.text(xtxt,ytxt, text ,color='k', size=size, horizontalalignment='right')
        pylab.show()
        return
    ...
    def write_members(self):
        ...
        head = "# %-23s %15s %15s  %6s %6s %6s %6s %6s  %6s  %6s  %6s  %6s  %6s \n" % ("ID","RA","DEC","ZB","TB","ZML","TML","g_mag","g_err","r_mag","r_err","i_mag","i_err")
        m.write(head)
        for i in self.iRadius[0]:
...
def KEfit(modelfile):
    ...
    (z,
     ...
     e_r,
     e_i,
     e_z) = tableio.get_data(modelfile,cols=(0,10,11,12,13,14,15,16,17))
...
def mi_star(z,cosmo=(0.3,0.7,0.7)):
    ...
    Mi_star = cosmology.reobs('El_Benitez2003',m=Mb_star, oldfilter="B_Johnson", newfilter="i_MOSAICII")
...
def cmdline():
    ...
    parser.add_option("--path",
                      dest="path", default=os.path.join(os.environ['HOME'],"SOAR-data/COMB"),
                      help="Path to data")

    parser.add_option("--radius",
                      dest="radius", default=1000.0,
                      help="Radius in kpc")

    parser.add_option("--zo",
                      dest="zo", default=None,
                      help="zo of cluster")

    parser.add_option("--dz",
                      dest="dz", default=0.08,
                      help="dz of shell")

    parser.add_option("--zuse",
                      dest="zuse", default="ZB",
                      help="use ZB or ML")
