...
class combcat:
        ...
        for line in open(self.assocfile).readlines():
            ...
            for filter in self.filters:
                if filter == filtername:
                    ...
                    self.files_weight[filter].append("%s.weight.fits'[0]'" % os.path.splitext(fname)[0])
    ...
    def copyfiles(self,copy="yes"):
        ...
        for file in self.infiles:
            cmd1 = "rsync -av -e ssh --progress %s ."     % os.path.join(self.datapath,file)
            print cmd1
            if copy:
                os.system(cmd1)
    ...
    def center_dither(self,conf="SWarp-center.conf"):
        ...
        opts = {}
        opts["IMAGEOUT_NAME"]   = os.path.join(self.outdir,"SWarp-%s-center.fits" % (self.tilename))
    ...
    def swarp_files(self,conf="SWarp-common.conf",dryrun=None,combtype="MEDIAN",reSWarp=None,filters=None):
        ...
        for filter in filters:
            ...
            self.weightima[filter] = "%s%s_weight.fits" % (self.tilename,filter)

            filelist = string.join(self.files[filter])
            cmd = "swarp %s -c %s -IMAGEOUT_NAME %s -WEIGHTOUT_NAME %s " % (filelist ,common_conf,outimage,outweight)
            cmd = cmd + " -WEIGHT_IMAGE %s " %  (",".join(self.files_weight[filter]))
            cmd = cmd + " -FSCALE_DEFAULT %s " % (",".join(map(str,self.flxscale[filter])))
            cmd = cmd + opts
    ...
    def header_FSCALE(self,filters=None,dm=0.05):
        ...
        for filter in filters:
            ...
            if len(self.files[filter]) < 2:
                print "Only one frame, no FLUX SCALE for %s %s filter" % (self.tilename,filter)
                continue
            ...
            if not self.do_level[filter]:
                print >>sys.stderr,"# No need to correct frames %s for %s" % (self.tilename, filter)
                continue
    ...
    def swarp_weight(self,conf="SWarp-common.conf",dryrun=None,combtype="SUM",filters=None):
    ...
    def generate_masks(self,filters,dryrun):
        ...
        for filter in filters:
            ...
            print >>sys.stderr,"# Generating mask image from %s --> %s" % (weight,mask)
    ...
    def make_swarp_input_weights(self,clobber=True):
        ...
        for fname,dqmask in self.dqmask.items():
            ...
            print >>sys.stderr,"# Generating weight image from %s --> %s" % (dqmask,outname)
    ...
    def SEx(self,det_filter='i'):
        ...
        self.combcat = {}
        self.SExinpar = os.path.join(os.environ['BCSPIPE'],'LIB/pars/bcs_Catalog.inpar')
        ...
        if self.DetImage:
            ...
            print>>sys.stderr, "# Will use %s band Detection Image:%s " % (det_filter,det_ima)
        ...
        for filter in self.filters:
            ...
            opts = opts + ' -WEIGHT_IMAGE %s%s_weight.fits ' % (self.tilename,det_filter)

            # Do the SEx
            cmd = "sex %s,%s -CATALOG_NAME %s -MAG_ZEROPOINT %s -c %s %s 1>&2" % (det_ima,input,output,self.magbase,self.SExinpar,opts)
	...
	for key in outColumns:
        ...
        detectionColumns    = tuple(detectionList)     # the get_data function requires a tuple
	detection_variables = tableio.get_data(detCatalog,detectionColumns)
        ...
        for filter in self.filters:
            ...
            fluxColumns = tuple(fluxList)    # the get_data function interface requires a tuple

            # Get the array using tableio
            flux[filter],fluxerr[filter] = tableio.get_data(self.combcat[filter],fluxColumns)
            ...
            nondetected = Numeric.less_equal(flux[filter],0.0) *\
                Numeric.greater(fluxerr[filter],0.0)
        ...
        flux[filter]=Numeric.clip(flux[filter],1e-100,1e100)
            m[filter]  = Numeric.where(detected,-2.5*Numeric.log10(abs(flux[filter]))+zpoint-self.XCorr[filter],m[filter])
        ...
        fluxerr[filter]=Numeric.clip(fluxerr[filter],1e-100,1e100)
            em[filter]  = Numeric.where(detected,2.5*Numeric.log10(1.0+abs(fluxerr[filter]/flux[filter]))+self.XCorrError[filter],em[filter])
            em[filter]  = Numeric.where(nondetected,2.5*Numeric.log10(abs(fluxerr[filter]))-zpoint,em[filter])
    ...
    tableio.put_data(self.colorCat,variables,header=header,format=format,append='no')
...
def replace_vals_image(infits,outfits,repval=1):
    ...
    Nima    = len(hdulist)
    print >>sys.stderr,"Replacing with n=%s on %s images, %s --> %s" % (repval,Nima-1,infits,outfits)
...
def put_scaled_head(file):
    ...
    #print >>sys.stderr,"# Updating %s with EXPTIME=%s after SWarp" % (file,exptime)
    c = pyfits.Card("FSCALED",True,"Has the flux been scaled in individual frames")
    c.verify()
...
def put_zeropt(file,zeropt,photo='yes'):
    ...
    print >>sys.stderr,"# Updating %s with ZP=%s as in SExtractor, photo:%s" % (file,zeropt,photo)

    after = "DATE"

    c = {}
    if photo == 'yes':
        c['ZEROPT']   = pyfits.Card("ZEROPT",zeropt,"Computed ZEROPOINT AB mags/sec")
        c['PHOTOM']   = pyfits.Card("PHOTOM",1,"Photometric quality 1=photo/0=not")
    else:
        c['ZEROPT']   = pyfits.Card("ZEROPT",zeropt,"Non-photo ZEROPOINT AB mags/sec")
        c['PHOTOM']   = pyfits.Card("PHOTOM",0,"Photometric quality 1=photo/0=not")
...
def chtype_replace_nonzero(infits, repval=1, out=None, verb=None):
    ...
    if verb:
        print >>sys.stderr, "\tChange type UInt8 on %s --- Replacing pixels not eq 0  --> %s" % (infits,repval)
...
def clean_FLXCORR(file,N=16):
    ...
    try:
        ...
        for i in range(N):
            n = i+1
            image = "%s[%s]" % (file,n)
            iraf.hedit(image,'FLXCORR',delete='yes',verify='no',update='yes', show='yes')
...
def cmdline():
    ...
    parser.add_option("--noCopy",
                      action="store_true", dest="noCopy", default=0,
                      help="No Copy files")

    parser.add_option("--noSWarp",
                      action="store_true", dest="noSWarp", default=0,
                      help="No SWarp")

    parser.add_option("--noBPZ",
                      action="store_true", dest="noBPZ", default=0,
                      help="No BPZ")

    parser.add_option("--noSEx",
                      action="store_true", dest="noSEx", default=0,
                      help="No SEx")

    parser.add_option("--Dust",
                      action="store_true", dest="Dust", default=0,
                      help="Dust Extinction Correction")

    parser.add_option("--WeightOnly",
                      action="store_true", dest="WeightOnly", default=0,
                      help="Create custom weights from images and exits")

    parser.add_option("--Weight",
                      action="store_true", dest="Weight", default=0,
                      help="Create custom weights from images")


    parser.add_option("--noMask",
                      action="store_true", dest="noMask", default=0,
                      help="Do not generate the mask from weight")

    parser.add_option("--useMask",
                      action="store_true", dest="useMask", default=0,
...
def main():
    ...
    # Init the class
    c = combcat(assocfile,datapath=inpath,outpath=outpath,verb='yes',dryrun=opt.dryrun)
    ...
    # SWarp
    c.swarp_files(dryrun=opt.noSWarp,conf="SWarp-common.conf",combtype=opt.combtype) #,filters=opt.frames_filters)
