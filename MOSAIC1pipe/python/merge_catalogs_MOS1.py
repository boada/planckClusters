    ...
    for ID in c.cat['i']['SHAPE'].keys():
        ...
        if goodID:
            ...
            for filter in filters:
                ...
                try:
                    mcat.write("%8.3f " % (c.cat[filter]['MAG_AUTO'][ID]  - c.Xcorr[filter][ID]))
                    mcat.write("%8.3f " % (c.cat[filter]['MAGERR_AUTO'][ID] + c.XcorrErr[filter][ID]))
                except:
                ...
                if (c.cat[filter]['MAGERR_AUTO'][ID] > 100):
                    ...
                    SN = 1./( 10**(0.4*c.cat[filter]['MAGERR_AUTO'][ID]) - 1)
                mcat.write("%8.3f " % SN)
            ...
            mcat.write("%8.2f " % (cat['A_IMAGE'][ID]*cat['KRON_RADIUS'][ID]) )
            mcat.write("%8.2f " % (cat['B_IMAGE'][ID]*cat['KRON_RADIUS'][ID]) )
...
def make_header(tilename,path):
    ...
    extra_keys = ['Z_B      Bayesian photo-z',
                  'Z_B_MIN  Min photo-z',
                  'Z_B_MAX  Max photo-z',
                  'T_B      Bayesian Spectral Type'     ,
                  'ODDS     Odds',
                  'Z_ML     Likelihood photo-z',
                  'T_ML     Likelihood Spectral Type',
                  'CHI-SQUARED',
                  'CLASS_STAR (1=star, 0=galaxy)',
                  'A_IMAGE x KRON_RADIUS (pixels)',
                  'B_IMAGE x KRON_RADIUS (pixels)',
                  'THETA_IMAGE (degrees)',
                  'X_IMAGE (pix)',
                  'Y_IMAGE (pix)']
