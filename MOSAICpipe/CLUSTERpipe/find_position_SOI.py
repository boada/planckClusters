...
class finder(object):
    ...
    def read_cat(self):
        ...
        #g_mask   = np.where(lor( g_bpz == 99,g_bpz == -99),0,1)
        #r_mask   = np.where(lor( r_bpz == 99,r_bpz == -99),0,1)
        #i_mask   = np.where(lor( i_bpz == 99,i_bpz == -99),0,1)
    ...
    def jpg_read(self):
        ...
        self.dx = old_div(self.nx, 2.0)
        self.dy = old_div(self.ny, 2.0)
        self.xcenter = old_div(self.nx, 2.0)
        self.ycenter = old_div(self.ny, 2.0)
        self.xo = old_div(self.nx, 2.0)
        self.yo = old_div(self.ny, 2.0)
        return
    ...
    def select_members(self, i, Mi_lim=-20.25):
        ...
        while not converge:
            ...
            c1 = np.abs(gr[idc] - gr[idc].mean()) > Nsigma * np.std(gr[idc],
                                                                    ddof=1)
            c2 = np.abs(ri[idc] - ri[idc].mean()) > Nsigma * np.std(ri[idc],
                                                                    ddof=1)
        ...
        c1 = np.abs(self.gr - gr[idc].mean()) > Nsigma * np.std(gr[idc],
                                                                ddof=1)
        c2 = np.abs(self.ri - ri[idc].mean()) > Nsigma * np.std(ri[idc],
                                                                ddof=1)
    ...
    def select_members_radius(self, i, Mi_lim=-20.25, radius=500.0, zo=None):
        ...
        R = radius  # in kpc
        r = old_div(
            astrometry.kpc2arc(zo, R, self.cosmo), 3600.)  # in degrees.
        rcore = old_div(r, 2.0)
        dist = astrometry.circle_distance(ra0,
                                          dec0,
                                          self.ra,
    ...
    def area_in_circle(self, xo, yo, r_pixels):
        ...
        d = np.sqrt((xo - ix)**2 + (yo - iy)**2)
        pix_in = np.where(d <= r_pixels)
        area_in = float(len(pix_in[0]))
        area_tot = math.pi * r_pixels**2
        self.area_fraction = old_div(area_in, area_tot)
        return
    ...
    def get_nearest(self, x, y):
        distance = np.sqrt((self.x_image - x)**2 + (self.y_image - y)**2)
        self.iclose = np.argmin(distance)
    ...
    def print_info(self):
        ...
        ra = astrometry.dec2deg(old_div(self.ra[i], 15.))
    ...
    def write_redshift(self):
        ...
        ds = 0.001
        x = np.arange(zx[2], zx[-3], ds)
        y = scipy.interpolate.splev(x, tck, der=0)
        y = np.where(y < 0, 0, y)

        # Get the 1 sigma error bars (68.2%)
        Psum = np.cumsum(y * ds / dz)
        i1 = np.where(Psum >= 0.159)[0][0]
        i2 = np.where(Psum > 0.841)[0][0]
        z1_68 = x[i1]
        z2_68 = x[i2]
        dz1 = zo - x[i1]
        dz2 = x[i2] - zo
        ...
        i1 = np.where(Psum >= 0.023)[0][0]
        i2 = np.where(Psum > 0.977)[0][0]
        z1_95 = x[i1]
        z2_95 = x[i2]
        dz1 = zo - x[i1]
        dz2 = x[i2] - zo
...
def PEllipse(xxx_todo_changeme,
    ...
    t = 2 * pi / resolution * np.arange(resolution)
    xtmp = A * np.cos(t)
    ytmp = B * np.sin(t)
...
def PCircle(xxx_todo_changeme2, radius, resolution=100, **kwargs):
    ...
    sin = math.sin
    t = 2 * pi / resolution * np.arange(resolution)
    xtmp = radius * np.cos(t)
    ytmp = radius * np.sin(t)
    x = xtmp + xo
    y = ytmp + yo
    return Polygon(list(zip(x, y)), **kwargs)
