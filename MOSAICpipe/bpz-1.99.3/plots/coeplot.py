...
def singlevalue(x):
    ...
    return type(x) in [
        NoneType, float, float32, float64, int, int0, int8, int16, int32, int64
...
def ploterrorbars(x,
    ...
    for i in range(len(x)):
        ...
        ax.plot([x[i] - dx, x[i] + dx], [ymax[i], ymax[i]],
                color=color,
                **other)
        ax.plot([x[i] - dx, x[i] + dx], [ymin[i], ymin[i]],
                color=color,
                **other)
...
def rectangle(lolimits, hilimits, fillit=0, **other):
    ...
    [xmax, ymax] = hilimits
    if not fillit:
        return plot([xmin, xmin, xmax, xmax, xmin],
                    [ymin, ymax, ymax, ymin, ymin], **other)
    else:
        if 'color' in list(other.keys()):
            color = other['color']
            del other['color']
            color = color2hex(color)
            return fill([xmin, xmin, xmax, xmax, xmin],
                        [ymin, ymax, ymax, ymin, ymin], color, **other)
        else:
            return fill([xmin, xmin, xmax, xmax, xmin],
                        [ymin, ymax, ymax, ymin, ymin], **other)
...
def atobplotold(xa,
    ...
    for i in range(n):
        ...
        if linetype == 'arrow':
            ...
            hx = 0.1 * hx
            hy = 0.1 * hy
            plot([xb[i], xb[i] + hx * hyfac], [yb[i], yb[i] + hy * hyfac],
                 color=color,
                 **other)
            plot([xb[i], xb[i] - hy * hyfac], [yb[i], yb[i] + hx * hyfac],
                 color=color,
                 **other)
        if linetype == 'xo':
            plot(xa,
...
def atobplot(xa,
    ...
    for i in range(n):
        ...
        if linetype == 'arrow':
            ...
            plot([xb[i], xb[i] + hx * hxfac], [yb[i], yb[i] + hy * hyfac],
                 color=color,
                 **other)
            plot([xb[i], xb[i] - hy * hxfac], [yb[i], yb[i] + hx * hyfac],
                 color=color,
                 **other)
        if linetype == 'xo':
...
def colormap_obsolete(x,
    ...
    for i in range(n):
        if not (isNaN(z[i]) or (valrange[0] == valrange[1])):
            plot([x[i]], [y[i]],
                 'o',
                 markerfacecolor=colormaprgb(z[i], valrange=valrange),
