...
class params(object):
    ...
    def check(self):
        ...
        view_keys(self.d)
        paso1 = eval(input('Do you want to change any parameter?(y/n)\n'))
        while paso1[0] == 'y':
            key = eval(input('Which one?\n'))
            if key not in self.d:
                paso2 = eval(input("This parameter is not in the dictionary.\
Do you want to include it?(y/n)\n"))
                if paso2[0] == 'y':
                    value = eval(input('value(s) of ' + key + '?= '))
                    self.d[key] = tuple(split(replace(value, ',', ' ')))
                else:
                    continue
            else:
                value = eval(input('New value(s) of ' + key + '?= '))
                self.d[key] = tuple(split(replace(value, ',', ' ')))
            view_keys(self.d)
            paso1 = eval(input('Anything else?(y/n)\n'))
    ...
    def run(self):
        ...
        if self.reject_fraction == None:
            for i in range(self.n):
                ...
                aver = median(xs)
                std1 = std(xs)
                good = good * less_equal(
                    abs(self.x - aver), self.n_sigma * std1)
                nnx = sum(good)
                if nnx == nx: break
                else: nx = nnx
        else:
...
def pointsrobust(x, y, limits=(None, None, None, None), title='Plot'):  # --DC
    ...
    if sum(outliers):
        ...
        if sum(toolo):
            ...
            p.add(Points(xolo,
                         yolo - dy,
                         type='half filled inverted triangle'))
    else:
...
def test():
    ...
    lines = ["This is a test header line", "This is another test header line",
