...
def sky_median_sig_clip(input_arr,
                        sig_fract,
                        percent_fract,
                        max_iter=100,
                        low_cut=True,
...
def sky_mean_sig_clip(input_arr,
                      sig_fract,
                      percent_fract,
                      max_iter=100,
                      low_cut=True,
...
def range_from_zscale(input_arr,
    ...
    while (((math.fabs(old_slope - new_slope) / new_slope) > percent_fract) and
           (iteration < max_iter)) and (len(y) >= midpoint_ind):
        iteration += 1
        old_slope = new_slope
        # clipping
        sig = y.std()
...
def histeq(inputArray, num_bins=1024):
    ...
    @type num_bins: int
    @param num_bins: number of bins in which to perform the operation (e.g.
                    1024)
    @rtype: numpy array
...
def power(inputArray, power_index=3.0, scale_min=None, scale_max=None):
    ...
    imageData[indices0] = 0.0
    imageData[indices2] = 1.0
    imageData[indices1] = numpy.power(
        (imageData[indices1] - scale_min), power_index) * factor
...
def asinh(inputArray, scale_min=None, scale_max=None, non_linear=2.0):
    ...
    imageData[indices0] = 0.0
    imageData[indices2] = 1.0
    imageData[indices1] = numpy.arcsinh(
