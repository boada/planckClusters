...
def within(A, xc, yc, ro, yesorno=0):  # --DC
    ...
    if yesorno:
        ...
        v = v + between(ro - 0.5 * dr, r, ro + 0.5 * dr) * (ro + 0.5 * dr - r
                                                            ) / dr
...
def atanxy(x, y, degrees=0):
    ...
    theta = where(
        logical_and(
            greater(x, 0), less(y, 0)), theta + 2 * pi, theta)
    if degrees:
        theta = theta * 180. / pi
    return theta
...
def singlevalue(x):
    ...
    return type(x) in [type(None), float, float32, float64, int, int0, int8,
                       int16, int32, int64
...
def median(m):
    ...
    if m.shape[0] & 1:
        ...
        return old_div((msort(m)[old_div(m.shape[0], 2)] + msort(m)[old_div(m.shape[0], 2) - 1]
...
def kaiser(M, beta):
    ...
    return old_div(cephes.i0(beta *
                     sqrt(1 - (old_div((n - alpha), alpha))**2.0)), cephes.i0(beta))
