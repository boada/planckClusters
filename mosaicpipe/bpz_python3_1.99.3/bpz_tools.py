...
def flux(xsr, ys, yr, ccd='yes', units='nu'):
    ...
    if units == 'nu':
        lp = sqrt(old_div(norm, trapz(yr / xsr / xsr, xsr)))  #Pivotal wavelenght
...
def etau(wl, z):
    ...
    for i in range(len(l)):
        i2 = searchsorted(wl[i1:i2], l[i] * xe)
        tau[i1:i2] = tau[i1:i2] + c[i] * (old_div(wl[i1:i2], l[i]))**(1. + gamma)
...
def nf_z_sed(sed,
    ...
    for i in range(nz):
        i1, i2 = n1[i], n2[i]
        ys_z = match_resol(x_sed[i1:i2], y_sed[i1:i2], old_div(x_r, (1. + z[i])))
...
def ABflux(sed, filter, madau='yes'):
    ...
    for i in range(nz_ab):
        ...
        if (x_sed[i1] > old_div(x_r[-1],
            (1. + z_ab[i]))) or (x_sed[i2 - 1] < old_div(x_r[0],
                                (1. + z_ab[i]))) or (i2 - i1 < 2):
            ...
            print(min(old_div(x_r, (1. + z_ab[i]))), max(old_div(x_r, (1. + z_ab[i]))))
            ...
            try:
                ...
                print(min(old_div(x_r, (1. + z_ab[i]))), max(old_div(x_r, (1. + z_ab[i]))))
                print(old_div(x_r[1], (1. + z_ab[i])), old_div(x_r[-2], (1. + z_ab[i])))
                print(x_sed[i1:i2])
...
class p_c_z_t(object):
    def __init__(self, f, ef, ft_z):
        ...
        self.foo = add.reduce(where(less(old_div(f, ef), 1e-4), 0., (old_div(f, ef))**2))
        ...
        self.ftt = add.reduce(
            ...
            where(nonobs, 0., old_div(ft_z**2, reshape(ef, (1, 1, self.nf))**2)),
            -1)
        ...
        self.chi2 = where(
            equal(self.ftt, 0.), self.foo,
            self.foo - old_div((self.fot**2), (self.ftt + eps)))
        self.chi2_minima = loc2d(self.chi2[:self.nz, :self.nt], 'min')
        self.i_z_ml = self.chi2_minima[0]
        self.i_t_ml = self.chi2_minima[1]
        self.min_chi2 = self.chi2[self.i_z_ml, self.i_t_ml]
        self.likelihood = exp(-0.5 * clip(
            (self.chi2 - self.min_chi2), 0., -2 * eeps))
        #self.likelihood=where(equal(self.chi2,1400.),0.,self.likelihood)
...
class p_c_z_t_color(object):
    def __init__(self, f, ef, ft_z):
        ...
        self.min_chi2 = self.chi2[self.i_z_ml, self.i_t_ml]
        self.likelihood = exp(-0.5 * clip(
            (self.chi2 - self.min_chi2), 0., 1400.))
...
def prior(z, m, info='hdfn', nt=6, ninterp=0, x=None, y=None):
    ...
    m_step = 0.1
    accuracy = str(len(str(int(old_div(1., m_step)))) - 1)  #number of decimals kept
...
class bpz_diagnosis(object):
    ...
    def stats(self,
        ...
        self.d = compress(self.good, old_div((self.zb - self.zs), (1. + self.zs)))
...
#        print self.zb[:10],self.odds[:10],self.zm[:10],self.zs[:10],self.mo[:10]
    ...
    def stats(self, odds_thr=(0.95, 0.99), z_lim=(0., 10.)):
        ...
        for i in range(len(odds_thr)):
            ...
            #Total Fraction of zb with dz larger than 0.06(1+zs)
            f_zbo_0p06 = old_div(sum(greater(abs(dzbo), 3 * 0.06)), float(len(dzbo)))
...
def test():
    ...
    nu = arange(old_div(1., x[-1]), old_div(1., x[0]), 1. / x[0] / 1e2) * clight_AHz
